...
RUN apt-get update     && apt-get install -y --no-install-recommends        ca-certificates        avahi-daemon        cups        cups-client        cups-filters        cups-ipp-utils        dbus-x11        evince        ghostscript        gnupg        inotify-tools        libasound2        libcups2        libgtk-3-0        libx11-6        libxext6        libxi6        libxrender1        libxtst6        xauth        xclip        unzip        xdg-utils        zenity     && apt-get clean     && rm -rf /var/lib/apt/lists/*
...
COPY setup_minimal_cups_printer.sh /usr/local/bin/setup_minimal_cups_printer.sh
