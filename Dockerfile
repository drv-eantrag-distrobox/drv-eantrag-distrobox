...
RUN apt-get update     && apt-get install -y --no-install-recommends        ca-certificates        cups-client        dbus-x11        gnupg        libasound2        libcups2        libgtk-3-0        libx11-6        libxext6        libxi6        libxrender1        libxtst6        xauth        xclip        unzip        zenity     && apt-get clean     && rm -rf /var/lib/apt/lists/*
